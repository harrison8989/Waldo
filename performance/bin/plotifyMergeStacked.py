...
def plotify():
    ...
    fileName = 'clientLog'
    labels = []
    colors = ['#ffbbbb','#bbffbb','#bbbbff','#ddddbb','#ddbbdd','#bbdddd','#cccccc']
    ...
    for folder in folders:
        ...
        x.append(average)
        xBar = ax.bar(0, average, 1, bottom=0, color = colors[foldCount])
        foldCount += 1
        xBars.append(xBar)
        previous = average
    ...
    for eks in x:
        log = math.log10(eks)
        logs.append(log)
        plt.text(.5, eks, round(eks, int(3 - log)),ha='center',va='bottom')
    #plt.title('Average Per Event Latency')
    plt.ylabel('Seconds per event')
    plt.gca().set_xlim(left=0)
    plt.gca().set_ylim(bottom=0,top=x[0])
