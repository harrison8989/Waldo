...
def Server (_waldo_classes,_host_uuid,_conn_obj,*args):
    class _Server (_waldo_classes["Endpoint"]):
        ...
        def _endpoint_func_call_prefix__waldo__sendMSG(self,_active_event,_context,msg,num,_returning_to_public_ext_array=None):
            ...
            thing = ""
            _tmp0 = (_context.get_val_if_waldo(thing,_active_event) + _context.get_val_if_waldo('3' ,_active_event))
            if not _context.assign(thing,_tmp0,_active_event):
                thing = _tmp0

            sing = ""
            _tmp0 = (_context.get_val_if_waldo(sing,_active_event) + _context.get_val_if_waldo('4' ,_active_event))
            if not _context.assign(sing,_tmp0,_active_event):
                sing = _tmp0

            bling = _context.get_val_if_waldo((_context.get_val_if_waldo(thing,_active_event) + _context.get_val_if_waldo(sing,_active_event)),_active_event)
