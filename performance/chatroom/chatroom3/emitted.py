    ...
    class _Client (_waldo_classes["Endpoint"]):
        ...
        def send_msg(self,texts):
            ...
            while True:  # FIXME: currently using infinite retry 
                ...
                try:
                    _to_return = self._endpoint_func_call_prefix__waldo__send_msg(_root_event,_ctx ,texts,[])
        ...
        def _endpoint_func_call_prefix__waldo__send_msg(self,_active_event,_context,texts,_returning_to_public_ext_array=None):
            if _context.check_and_set_from_endpoint_call_false():
                texts = _context.turn_into_waldo_var_if_was_var(texts,True,_active_event,self._host_uuid,False,False)

                pass

            else:
                texts = _context.turn_into_waldo_var_if_was_var(texts,False,_active_event,self._host_uuid,False,False)

                pass

            (self._partner_endpoint_msg_func_call_prefix__waldo__SeqSendMSG(_active_event,_context,texts,) if _context.set_msg_send_initialized_bit_false() else None)

        ### USER DEFINED SEQUENCE BLOCKS ###

        ### User-defined message send blocks ###

        def _partner_endpoint_msg_func_call_prefix__waldo__SeqSendMSG(self,_active_event,_context,msgs=None,_returning_to_public_ext_array=None):
            ...
            if not _context.set_msg_send_initialized_bit_true():
                ...
                if _context.check_and_set_from_endpoint_call_false():
                    ...
                    _context.sequence_local_store.add_var(
                        "4__msgs", _context.convert_for_seq_local(msgs,_active_event,self._host_uuid)
                    )
...
def Server (_waldo_classes,_host_uuid,_conn_obj,*args):
    class _Server (_waldo_classes["Endpoint"]):
        ...
        def _partner_endpoint_msg_func_call_prefix__waldo__receiveMSG(self,_active_event,_context,_returning_to_public_ext_array=None):
            ...
            _context.signal_call(_active_event,_context.global_store.get_var_if_exists("2__display_msg_func"),_context.sequence_local_store.get_var_if_exists("4__msgs"),)
